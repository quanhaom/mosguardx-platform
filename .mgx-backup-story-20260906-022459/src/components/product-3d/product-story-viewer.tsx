...
function StorySwarm({
  ...
  useFrame((_, delta) => {
    ...
    Array.from({ length: MOSQUITO_COUNT }).forEach((_, index) => {
      ...
      const wobble =
        elapsed.current * (2.7 + seeded(index, 2)) + seeded(index, 3) * 8;

      let radius = progress < 0.54 ? 0.07 : 0.16;
      if (progress > 0.84) radius = 0.11;

      const ppfThreshold = 0.43 + seeded(index, 7) * 0.012;
      if (progress >= ppfThreshold) ppfContacted.current[index] = true;
      if (ppfContacted.current[index] && progress < 0.67) radius *= 0.22;

      point.x += Math.cos(wobble) * radius;
      point.y += Math.sin(wobble * 1.25) * radius * 0.6;
      point.z += Math.sin(wobble) * radius * 0.55;

      const visibleScale = progress > 0.001 && progress < 0.995 ? 1 : 0;
      const size = (0.032 + seeded(index, 8) * 0.008) * visibleScale;
      ...
      bodyRef.current!.setColorAt(
        index,
        carriesPpf ? BODY_PURPLE : BODY_BLACK,
      );
      leftWingRef.current!.setColorAt(
        index,
        carriesPpf ? WING_PURPLE : WING_NORMAL,
      );
      ...
      dummy.scale.set(size * 0.34, size, size * 0.34);
      dummy.updateMatrix();
      bodyRef.current!.setMatrixAt(index, dummy.matrix);

      const flap = 0.45 + Math.abs(Math.sin(elapsed.current * 33 + index)) * 0.68;

      dummy.position.copy(point);
      dummy.quaternion.setFromUnitVectors(LOCAL_FORWARD, tangent);
      dummy.rotateZ(flap);
      dummy.scale.set(size * 0.82, size * 0.08, size * 0.38);
      dummy.updateMatrix();
